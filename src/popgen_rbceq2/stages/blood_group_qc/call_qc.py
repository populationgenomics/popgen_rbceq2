...
class FlagBloodGroupCallQc(cpg_flow.stage.SequencingGroupStage):
    ...
    stage that reports them rather than filtering on them.
    ...
    def queue_jobs(
        ...
        args: dict[str, stage_support.JobArg] = {
            ...
            'min-depth': str(cpg_utils.config.config_retrieve(['workflow', cfg, 'min_depth'], 10)),
            'min-gq': str(cpg_utils.config.config_retrieve(['workflow', cfg, 'min_gq'], 20)),
        }
        ...
        # Analysis, so record the path rather than leaving the two records unlinked.
        # update_analysis_meta only receives the output path and could not derive it.
        return self.make_outputs(
            sequencing_group,
            data=outputs,
            jobs=[j],
            meta={'blood_group_genotypes_path': geno_tsv},
        )
