...
- SV VCF / CNV VCF: DRAGEN's two structural-variant outputs per sequencing group, beside the gVCF. The SV VCF is written by the DRAGEN SV caller, which integrates and extends Manta (breakpoint evidence, events of any size, proper `SVTYPE`; record IDs keep Manta's prefix); the CNV VCF is bin-based read-depth (dosage, `SVTYPE=CNV` on every record, events under 10 kb filtered as `cnvLength`). Genome runs produce both. Production exome runs produce both too, but the exome CNV VCF is called per capture target against a panel of normals and holds events only (no `DRAGEN:REF:` records, no `cnvLength` filter). Neither is an input to this pipeline yet; see `docs/rbceq2_cnv_sv/`.
